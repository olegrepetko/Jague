...
namespace Jague.Common.Network.Message
{
    ...
    public sealed class MessageManager : Singleton<MessageManager>
    {
        ...
        private ImmutableDictionary<MessageOpcode, MessageHandlerDelegate> clientMessageHandlers;
        ...
        public bool GetOpcode(IWritable message, out MessageOpcode opcode)
        {
        ...
        {
            var messageFactories = new Dictionary<MessageOpcode, MessageFactoryDelegate>();
            var messageOpcodes = new Dictionary<Type, MessageOpcode>();
            ...
            log.Info($"Initialised {clientMessageFactories.Count} factory message(s)");
            log.Info($"Initialised {serverMessageOpcodes.Count} message(s).");
        }

        private void InitialiseMessageHandlers()
        {
            var messageHandlers = new Dictionary<MessageOpcode, MessageHandlerDelegate>();
        ...
        public MessageHandlerDelegate GetMessageHandler(MessageOpcode opcode)
        {
